...
def build_engine(onnx_file_path, engine_file_path, fp16=True):
    ...
    # Note: set_memory_pool_limit is an API for TRT 8.x+
...
def main():
    ...
    # 1. Try to load or build Engine
    if not os.path.exists(engine_path):
        print(f"Engine not found. Building from {onnx_path}...")
        engine = build_engine(onnx_path, engine_path, fp16=True)
    else:
        ...
        try:
            ...
            if engine is None:
                print("Engine deserialization returned None (likely version mismatch). Rebuilding from ONNX...")
                engine = build_engine(onnx_path, engine_path, fp16=True)

        except Exception as e:
            print(f"Error loading engine: {e}")
            print("The engine is likely incompatible. Rebuilding from ONNX...")
            engine = build_engine(onnx_path, engine_path, fp16=True)
